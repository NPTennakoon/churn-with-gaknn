            ...
            if (m_Data.Attribute(i).Selected()) {
                ...
                switch (m_Data.Attribute(i).Type()) {
                    case Attribute.NOMINAL:
                        if (bMissingValue) {
                            index = -1;
                        } else // Check if value appears in header.
                        {
                        ...
                        if (m_Data.Attribute(i).IsClassAttribute()) {
                            ...
                            if (bMissingValue) {
                                ...
                                // System.out.println("index v "+m_ValueHandler.GetValue(Attribute.NOMINAL, i, m_Tokenizer.sval));
                    ...
                    case Attribute.NUMERIC:
                        ...
                        try {
                            ...
                            //System.out.println("numIndex"+numIndex); //to tarce an error 15-07-2012
                    ...
                    case Attribute.DATE:
                        try {
                            ...
                            numIndex++;
//TODO : two increament here... confusing,,,
                            numIndex++;
    ...
    public void ReadDataRow() throws IOException {

        if (m_filePath.length() == 0) {
            throw new IOException("Missing File");
        } else {
            FileReader frr = new FileReader(m_filePath);
            BufferedReader brr = new BufferedReader(frr);

            m_Tokenizer = new StreamTokenizer(brr);
            initTokenizer();
            m_Lines = 0;

//            for(m_Lines = 0;m_Lines<=5;m_Lines++){
            // Get name of relation.
            getFirstToken();
            if (m_Tokenizer.ttype == StreamTokenizer.TT_EOF) {
                errorMessage("premature end of file");
...
//            }
    ...
    protected FastVector parseDataRows() throws IOException {
        String attributeName;
        FastVector datarows = new FastVector();
        FastVector rowValues;
        int numAttributes = 0;
        ...
        while (ARFF_ATTRIBUTE.equalsIgnoreCase(m_Tokenizer.sval)) {
            ...
            if (m_Tokenizer.ttype == StreamTokenizer.TT_WORD) {
                ...
                if (m_Tokenizer.sval.equalsIgnoreCase(ARFF_ATTRIBUTE_REAL)
                        ...
                        || m_Tokenizer.sval.equalsIgnoreCase(ARFF_ATTRIBUTE_NUMERIC)) {
                    numAttributes++;
                    datarows.addElement(new Attribute(attributeName, datarows.size(), numAttributes));
                    readTillEOL();
                } else if (m_Tokenizer.sval.equalsIgnoreCase(ARFF_ATTRIBUTE_STRING)) {
                    numAttributes++;
                    datarows.addElement(new Attribute(attributeName, (FastVector) null,
                            datarows.size(), numAttributes));
                    readTillEOL();
                } else if (m_Tokenizer.sval.equalsIgnoreCase(ARFF_ATTRIBUTE_DATE)) {
                    String format = null;
                    if (m_Tokenizer.nextToken() != StreamTokenizer.TT_EOL) {
                    ...
                    numAttributes++;
                    datarows.addElement(new Attribute(attributeName, format,
                            datarows.size(), numAttributes));
                } else {
                    errorMessage("no valid attribute type or invalid "
                            + "enumeration");
                ...
                // Attribute is nominal.
                rowValues = new FastVector(); //this fastvector stores the possible values for each nominal attribute
                // System.out.println("fast vector cap: "+rowValues.capacity());
                ...
                while (m_Tokenizer.nextToken() != '}') {
                    if (m_Tokenizer.ttype == StreamTokenizer.TT_EOL) {
                        ...
                        rowValues.addElement(m_Tokenizer.sval);
                    }
                }
                // System.out.println("fast vector cap2: "+rowValues.capacity()); //this fastvector stores the possible values for each nomila attribute

                Attribute attrb = new Attribute(attributeName, rowValues, datarows.size(), numAttributes);

                //System.out.println(attrb.Value(m_Lines)); //npt
                // if (attributeName.compareTo(Attribute.ARFF_ATTRIBUTE_CLASS) == 0){
                //    attrb.SetClassAttribute(true);
                // }
                //else
