...
public class DiscreteEstimatorBayes extends Estimator {
     ...
     *
     * @param nType score type Bayes is used
     * @return the score
     */
    public double logScore(int nCardinality) {
        double fScore = 0.0;
    ...
    public String toString() {
        ...
        if (m_SumOfCounts > 1) {
            for (int i = 0; i < m_Counts.length; i++) {
                result += " " + doubleToString(m_Counts[i], 2);
            }

            result += "  (Total = " + doubleToString(m_SumOfCounts, 2)
                    + ")\n";
        } else {
            for (int i = 0; i < m_Counts.length; i++) {
                result += " " + m_Counts[i];
